...
export default function TeacherAttendanceHistoryPage() {
  ...
  // Fetch sections taught/available in school
  useEffect(() => {
  ...
  // Fetch attendance ledger when filters change
  useEffect(() => {
    ...
    const fetchLedger = async () => {
      ...
      setError(null)
      try {
        const dateStr = format(date, 'yyyy-MM-DD') // formatting for backend
        const month = date.getMonth() + 1
        const year = date.getFullYear()

        let url = `/attendance?section_id=${selectedSectionId}&date=${format(date, 'yyyy-MM-dd')}`
        if (range !== 'daily') {
          url += `&range=${range}`
          if (range === 'monthly') {
    ...
    fetchLedger()
  }, [selectedSectionId, range, date])

  const filteredRecords = records.filter(r => 
    r.user.name?.toLowerCase().includes(searchQuery.toLowerCase()) || 
    r.user.students?.[0]?.rollNumber.toString().includes(searchQuery)
  )
  ...
  return (
    <div className="flex flex-col min-h-[100dvh] relative z-0 text-foreground">
      ...
      <div className="sticky top-0 z-10 px-6 pt-12 pb-6 flex flex-col gap-4">
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-xl font-black tracking-tight text-white">Attendance Ledger</h1>
            <p className="text-[10px] sm:text-xs font-semibold text-white/70 uppercase tracking-widest mt-0.5 truncate">
              Class History & Roster Analysis
            </p>
          </div>
        </div>
          ...
          <div className="flex gap-2">
            {sections.length > 0 && (
              <Select value={selectedSectionId} onValueChange={setSelectedSectionId}>
                <SelectTrigger className="w-[130px] h-11 rounded-2xl bg-background/60 shadow-sm border-border/50 text-sm font-semibold shrink-0">
            ...
            <Select value={range} onValueChange={(r: any) => setRange(r)}>
              <SelectTrigger className="w-[110px] h-11 rounded-2xl bg-background/60 shadow-sm border-border/50 text-sm font-semibold shrink-0">
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3 min-w-0 pr-4">
                    <div className="w-10 h-10 rounded-full bg-blue-500/10 text-[#0A4EA6] flex items-center justify-center font-bold text-sm shrink-0">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-[10px] text-muted-foreground mt-1 font-mono uppercase tracking-wider">
                        Roll: {record.user.students?.[0]?.rollNumber || 'N/A'}
                      </p>
                    </div>
                  </div>
      ...
      {/* Student Drill-Down Side-Panel/Modal */}
      {selectedStudent && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-background w-full sm:max-w-lg rounded-t-[2.5rem] sm:rounded-[2.5rem] border border-border/40 shadow-2xl p-6 space-y-6 animate-in slide-in-from-bottom-10 duration-300">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-xl font-black tracking-tight">{selectedStudent.user.name}</h2>
                <p className="text-xs text-muted-foreground font-semibold uppercase tracking-wider mt-1">
                  Class Statistics for {range} range
                </p>
              </div>
              <button
            ...
            <div className="grid grid-cols-2 gap-4 bg-muted/20 border border-border/30 rounded-3xl p-5">
              <div className="flex flex-col justify-center">
                <span className="text-3xl font-black text-primary">{selectedStudent.stats.percentage}%</span>
              ...
              <div className="grid grid-cols-2 gap-2 text-center text-xs font-bold">
                <div className="bg-emerald-500/5 border border-emerald-500/10 p-1.5 rounded-xl">
                  <p className="text-emerald-500 font-black">{selectedStudent.stats.present}</p>
                ...
                <div className="bg-red-500/5 border border-red-500/10 p-1.5 rounded-xl">
                  <p className="text-red-500 font-black">{selectedStudent.stats.absent}</p>
                ...
                <div className="bg-amber-500/5 border border-amber-500/10 p-1.5 rounded-xl">
                  <p className="text-amber-500 font-black">{selectedStudent.stats.leave}</p>
                ...
                <div className="bg-blue-500/5 border border-blue-500/10 p-1.5 rounded-xl">
                  <p className="text-blue-500 font-black">{selectedStudent.stats.holiday}</p>
            ...
            <ScrollArea className="max-h-[300px] w-full border border-border/40 rounded-2xl">
              <div className="p-3 space-y-2">
                {Array.isArray(selectedStudent.days) ? (
                  selectedStudent.days.map((day, idx) => {
                  ...
                  <div className="flex justify-between items-center bg-background border border-border/30 rounded-xl p-3">
                    <div>
                      <p className="text-sm font-bold">
                        {format(new Date((selectedStudent.days as AttendanceDay).date), 'EEEE, MMM d, yyyy')}
                      </p>
                      {(selectedStudent.days as AttendanceDay).title && (
                        <p className="text-[10px] text-blue-500 font-bold mt-0.5">{(selectedStudent.days as AttendanceDay).title}</p>
                      )}
                    </div>
                    <Badge className={cn("text-[9px] font-black uppercase border px-2 py-0.5", (statusConfig[(selectedStudent.days as AttendanceDay).status] || statusConfig.UNMARKED).style)}>
                      {(statusConfig[(selectedStudent.days as AttendanceDay).status] || statusConfig.UNMARKED).label}
                    </Badge>
                  </div>
                )}
              </div>
